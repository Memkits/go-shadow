...
(task-options!
  pom {:project     'Memkits/go-shadow
       :version     +version+
       :description "Workflow"
       :url         "https://github.com/Memkits/go-shadow"
       :scm         {:url "https://github.com/Memkits/go-shadow"}
...
(deftask dev []
  (comp
    ...
    (watch)
    (reload :on-jsload 'go-shadow.core/on-jsload)
    (cljs)
    (target)))
...
(deftask rsync []
  (fn [next-task]
    (fn [fileset]
      (sh "rsync" "-r" "target/" "tiye:repo/Memkits/go-shadow" "--exclude" "main.out" "--delete")
      (next-task fileset))))
